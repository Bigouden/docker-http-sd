...
HTTP_ADDRESS = os.environ.get("HTTP_ADDRESS", "0.0.0.0")  # nosec B104
...
if TARGETS_DELIMITER not in VALID_TARGETS_DELIMITER:
    logging.error(
        "INVALID TARGETS_DELIMITER: '%s' (VALID: '%s')",
        TARGETS_DELIMITER,
...
def discover():
    ...
    for container in containers:
        ...
        services = defaultdict(
            lambda: {"targets": [], "network": str(), "port": int(), "labels": []}
        )
            ...
            if (
                ...
                if label_split[2] == "targets":
                    services[service]["targets"] = (
                        re.sub(r"\s+", "", value).strip('"').split(TARGETS_DELIMITER)
                    )
                ...
                if label_split[2] == "port":
                    try:
                        ...
                        logging.error(
                            "INVALID PORT %s FOR SERVICE %s ON CONTAINER %s.",
                            value,
                            service,
                            container.name,
                        )
        ...
        for key, value in _["services"].items():
            ...
            try:
                ...
                logging.error(
                    "INVALID NETWORK %s FOR SERVICE %s ON CONTAINER %s.",
                    value["network"],
                    key,
                    container.name,
                )
...
class GetHandler(BaseHTTPRequestHandler):
    ...
    def _set_headers(self, code):
        ...
        self.send_header("Cross-Origin-Opener-Policy", "same-site")
        self.end_headers()
...
if __name__ == "__main__":
    ...
    httpd = HTTPServer((HTTP_ADDRESS, HTTP_PORT), GetHandler)
    httpd.serve_forever()
